# 끝에서 k번째 찾기 - 투포인터 기법

### [ 문제 정보 ]
> **기타**: https://fern-freeze-290.notion.site/2-aa9f6967b8604d3d9d609d0cc9ce0c9e#a517f3cae1844057a402799feb8959b1
> 
> **난이도**: 1
>
> **유형**: #투포인터/슬라이딩 윈도우


### [ 문제 풀이 ]
```JavaScript
class Node:
    def __init__(self, data):
        self.data = data
        self.next = None


class LinkedList:
    def __init__(self, value):
        self.head = Node(value)

    def append(self, value):
        cur = self.head
        while cur.next is not None:
            cur = cur.next
        cur.next = Node(value)

    # length 이용하여 문제 풀기
    # def get_kth_node_from_last(self, k):
    #     length = 1  # 시작 노드의 길이를 세기 위해 1부터 시작
    #     cur = self.head  # cur → [6]
    #
    #     while cur.next is not None:
    #         cur = cur.next # cur → [7], # cur → [8]
    #         length += 1 # # length = 2, # length = 3
    #     end_length = length - k # 3 - 2 = 1
    #     cur = self.head  # 다시 cur → [6]
    #     for i in range(end_length):  # range(1)
    #         cur = cur.next # 한 번 이동 → cur → [7]
    #     return cur # [7]

    # 투포인터 기법
    def get_node_from_last(self, k):
        slow = self.head
        fast = self.head
        print("slow 초기 :", slow.data)
        print("fast 초기 :", fast.data)

        for i in range(k):
            fast = fast.next
            # print(fast)
        # k 값이 1이라고 가정할 떄
        # slow : 6 , fase : 8
        while fast is not None:
            slow = slow.next # 7
            fast = fast.next # none
            # print("slow 중간",slow.data)
            # print("slow 중간",fast)

        print("slow 최종:",slow.data)
        # print("fast 최종:",fast.data)
        return slow

    # def get_node_from_last(self,k):
    #     # 끝에서 k번째 값 출력하기
    #     length = 1
    #     cur = self.head
    #     # print(cur.data)
    #
    #     while cur.next is not None:
    #         cur = cur.next
    #         # print(cur.data)
    #         length += 1
    #     end_length = length - k
    #     print("end_length :",end_length)
    #     cur = self.head
    #     for i in range(end_length):
    #         cur = cur.next
    #         print(cur.data)
    #     return cur

linked_list = LinkedList(6)
linked_list.append(7)
linked_list.append(8)


# print(linked_list.get_kth_node_from_last(2).data)
# linked_list.get_node_from_last(1)
print('최종값',linked_list.get_node_from_last(2).data)
# linked_list.get_node_from_last(2)
# linked_list.get_node_from_last(3)
```


---
@www.codrive.co.kr